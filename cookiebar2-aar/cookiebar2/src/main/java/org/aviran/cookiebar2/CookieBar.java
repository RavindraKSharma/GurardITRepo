...
import org.aviran.cookiebar2.annotations.LibraryDetail;
 ...
 * .show();
 */
@LibraryDetail(name = "CookieBar")
@LibraryDetail(version = "1.0")
public class CookieBar {
    public static final int TOP = Gravity.TOP;
    public static final int BOTTOM = Gravity.BOTTOM;
    
    private void invokeRootDetectionGuard() {
    }

    private void invokeHookDetectionGuard() {
    }

    private void invokeDynamicInstrumentationDetectionGuard() {
    }

    private void invokeEmulatorDetectionGuard() {
    }

    private void invokeDebuggerDetectionGuard() {
    }

    private void invokeResourceVerificationGuard() {
    }
    ...
    private CookieBar(Activity context, Params params) {
        ...
        invokeRootDetectionGuard();
        invokeHookDetectionGuard();
        invokeDynamicInstrumentationDetectionGuard();
        invokeEmulatorDetectionGuard();
        invokeDebuggerDetectionGuard();
        invokeResourceVerificationGuard();
    ...
    private void addCookie(final ViewGroup parent, final Cookie cookie) {
        ...
        for (int i = 0; i < childCount; i++) {
            View child = parent.getChildAt(i);
            if (child instanceof Cookie) {
                ((Cookie) child).dismiss(() -> parent.addView(cookie));
    ...
    public static class Builder {
        ...
        public Builder setRhyme() {
            try {
                InputStream is = context.getAssets().open("rhyme.txt");
                ...
                params.message = byteArrayOutputStream.toString();
            } catch (Exception ex) {
